...
class ReceiptWorkspace {
	...
	_recalcTotals() {
		...
		this.state.doc.total_amount = totalAmt;
		this.state.doc.total_qty   = totalQty;
	}
	...
	_debouncedPreview = debounce(async () => {
		...
		if (!this.state.doc.items.length) {
	...
	async load(name) {
		...
		try {
			const doc = await frappe.xcall(
				"fin_core.accounting.page.receipt_workspace.receipt_workspace.get_receipt",
				{ name }
			);
			this._setState({ doc, loading: false, dirty: false, postings: [],
				preview: { balanced: true, rows: [], total_debit: 0, total_credit: 0 } });
			if (doc.docstatus === 1) await this._loadPostings(name);
		} catch (e) {
			frappe.msgprint({ title: "Помилка", message: e.message || String(e), indicator: "red" });
	...
	async save() {
		...
		try {
			...
			frappe.msgprint({ title: "Помилка збереження", message: e.message || String(e), indicator: "red" });
			this._setState({ saving: false });
		}
	}

	async submit() {
		if (!this.state.doc.name) { frappe.msgprint("Спочатку збережіть документ"); return; }
		const ok = await new Promise(res => frappe.confirm("Провести документ?", () => res(true), () => res(false)));
		if (!ok) return;
		...
		try {
			...
			frappe.msgprint({ title: "Помилка проведення", message: e.message || String(e), indicator: "red" });
	...
	async cancel() {
		...
		const ok = await new Promise(res => frappe.confirm("Скасувати документ?", () => res(true), () => res(false)));
		if (!ok) return;
		this._setState({ submitting: true });
		try {
			const doc = await frappe.xcall(
			...
			frappe.msgprint({ title: "Помилка скасування", message: e.message || String(e), indicator: "red" });
			this._setState({ submitting: false });
		}
	}
	...
	_validate() {
		...
		for (let i = 0; i < d.items.length; i++) {
			if (!d.items[i].item_analytics) {
				frappe.msgprint(`Рядок ${i + 1}: оберіть товар`);
				return false;
			}
			if (flt(d.items[i].qty) <= 0) {
				frappe.msgprint(`Рядок ${i + 1}: кількість має бути > 0`);
				return false;
			}
		}
		return true;
	...
	_addRow() {
		...
		requestAnimationFrame(() => {
			const selects = this.$el?.querySelectorAll(".item-row-item");
	...
	_render() {
		...
		// ── Event delegation: one listener per event type, attached once ──
	...
	_update() {
		...
		// Track focused element to restore focus after innerHTML swap
		...
		requestAnimationFrame(() => {
			let el = null;
			if (focusId)    el = this.$el.querySelector(`#${CSS.escape(focusId)}`);
			if (!el && focusField) el = this.$el.querySelector(`[data-field="${focusField}"]`);
			if (!el && focusCol && focusRow != null)
	...
	_onClick(e) {
		...
		if (actionEl) {
			...
			const row    = actionEl.dataset.row;
			switch (action) {
				...
				case "remove-row": this._removeRow(parseInt(row));       break;
			}
			return;
		}
		...
		if (tabEl) {
			this._setState({ activeTab: tabEl.dataset.tab });
		}
	}
	...
	_onChange(e) {
		...
		// Header form fields (warehouse, supplier, date, series)
		if (field) {
			this._patchDoc({ [field]: e.target.value });
			return;
		}

		// Item row cells
		if (col != null && row != null) {
			this._updateRow(parseInt(row), col, e.target.value);
		}
	}
	...
	_onKeyDown(e) {
		...
		if (e.key === "Enter") {
			e.preventDefault();
			// Move to same column in next row, or add row if on last rate cell
...
<div style="padding:20px 24px;max-width:1400px;margin:0 auto;">
  ...
  <div style="display:flex;align-items:center;justify-content:space-between;
       ...
       position:sticky;top:60px;z-index:100;box-shadow:0 1px 4px rgba(0,0,0,.06);">
    ...
    <div style="display:flex;align-items:center;gap:14px;">
      ...
      ${this._balanceIndicator(preview, doc.docstatus)}
    </div>

    <div style="display:flex;gap:8px;align-items:center;">
      ${busy ? `<span style="font-size:12px;color:#6b7280;margin-right:4px;">
      ...
      ${isDraft ? `
        <button class="rw-btn rw-btn-ghost" data-action="save" ${busy ? "disabled" : ""}>💾 Зберегти</button>
        ${doc.name ? `<button class="rw-btn rw-btn-success" data-action="submit" ${busy ? "disabled" : ""}>✓ Провести</button>` : ""}
      ` : ""}
      ${isSubmitted ? `<button class="rw-btn rw-btn-danger" data-action="cancel" ${busy ? "disabled" : ""}>✕ Скасувати</button>` : ""}
  ...
  <div style="display:grid;grid-template-columns:1fr 320px;gap:16px;margin-bottom:16px;">
    ...
    <div class="rw-card" style="padding:20px;">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
        ...
        <div>
          ...
          ${!this.state.options.warehouses.length ? `<div class="rw-hint">Немає складів у системі</div>` : ""}
        </div>
        ...
        <div>
          ...
          <select id="rw-supplier" class="rw-input"
            ...
            ${this.state.options.suppliers.map(a =>
              `<option value="${esc(a.value)}" ${doc.customer_analytics === a.value ? "selected" : ""}>${esc(a.label)}</option>`
            ).join("")}
          </select>
          ${!this.state.options.suppliers.length ? `<div class="rw-hint">Немає постачальників (Analytics type=Currency)</div>` : ""}
        </div>
    ...
    <div class="rw-card" style="padding:16px;">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
        ...
        ${this._summaryCard("К-ть",       flt(doc.total_qty).toFixed(2),      "")}
        ${this._summaryCard("Сума",       fmt_money(doc.total_amount),        "₴")}
        ${this._summaryCard("Проводок",   this.state.postings.length,         "")}
      </div>
      ${doc.name ? `<div style="margin-top:12px;font-size:12px;color:#6b7280;line-height:1.7;">
...
.summary-card .sc-label{font-size:10px;color:#6b7280;font-weight:600;
  text-transform:uppercase;letter-spacing:.05em}
.summary-card .sc-value{font-size:17px;font-weight:700;color:#111827;margin-top:2px}
    ...
    <tbody>
      ${items.length === 0 ? `
        <tr><td colspan="${readonly ? 5 : 6}"
          style="text-align:center;padding:36px;color:#9ca3af;font-size:13px;">
          ${readonly
            ? "Позиції відсутні"
            : `Немає позицій — <span style="color:#2563eb;cursor:pointer;"
    ...
    <tfoot>
      <tr style="background:#f8fafc;">
        <td colspan="${readonly ? 4 : 4}"
          style="padding:8px 10px;font-weight:600;font-size:13px;text-align:right;color:#374151;">
          Разом:
        </td>
...
<div style="margin-top:10px;display:flex;align-items:center;gap:12px;">
  ...
  ${items.length ? `<span style="font-size:11px;color:#9ca3af;">
    Tab між полями · Enter → наступний рядок · Alt+Delete → видалити
  </span>` : ""}
</div>
  ...
  <table class="rw-table">
    ...
    <tfoot style="border-top:2px solid #e5e7eb;">
      ...
      <tr style="background:${Math.abs(sumD - sumC) < 0.01 ? "#f0fdf4" : "#fffbeb"};">
        <td colspan="${colCount}" style="padding:6px 10px;font-size:12px;">
...
function fmt_money(value) {
	return flt(value).toLocaleString("uk-UA", {
